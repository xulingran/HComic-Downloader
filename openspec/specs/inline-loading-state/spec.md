# inline-loading-state 规范

## 目的
定义页面在无旧结果可遮罩的场景下（如列表页首次加载、React.lazy Suspense fallback）展示居中加载态的统一渲染契约：必须使用共享 `InlineLoading` 组件渲染居中 spinner 环与可选辅助文案，避免在各页面内联重复编写 spinner JSX，并保证 spinner 视觉模式与全视口遮罩组件 `LoadingOverlay` 一致、尊重用户的 reduced-motion 偏好。
