# dev-server-connectivity 规范

## 目的
定义 Electron dev 模式下，应用主窗口可靠加载 Vite renderer dev server 的能力规范。要求 dev server 绑定 loopback 固定端口、强制 Electron session 对 loopback 直连（绕过系统/TUN 代理）、`loadURL` 前主动探测 dev server 就绪、以及首次加载失败时以指数退避重试，确保在系统代理或 TUN 模式激活时 dev server 首次加载仍可达，无需用户手动切换代理开关。仅约束 dev 模式（`ELECTRON_RENDERER_URL` 存在时），生产构建路径（`loadFile`）不受影响。
