...
- **当** 一个新增或修改的前端测试用例在 `tests/unit` 作用域内包含 `expect(x).toHaveBeenCalled()` 或 `expect(x).toHaveBeenCalledTimes(n)` 形式的断言，且同一 `it` 块内不存在任何对返回值（`expect(await fn()).toEqual(...)` / `resolves`）、抛错（`rejects.toThrow`）或可观察状态变化（`getState()` / DOM / `document` 属性）的断言
- **那么** 闸门**必须**报告该用例为失败，并提示"补充行为断言（返回值/状态/抛错）或删除该 mock 调用断言"

#### 场景:Python 裸 assert_called 被拦截

- **当** 一个新增或修改的 Python 测试用例包含 `mock.assert_called()` / `assert_called_once()` / `assert_called_with(...)` 形式的断言，且同一测试函数内不存在对返回值、异常（`pytest.raises` / `try-except`）或可观察状态（属性读取、文件内容、字典内容）的断言
- **那么** 闸门**必须**报告该用例为失败

#### 场景:伴随行为断言的 mock 断言被放行

- **当** 一个测试用例同时包含 mock 调用断言**与**真实的返回值/状态/抛错断言（例如 `main.test.ts` 中 `expect(app.quit).toHaveBeenCalled()` 与 `expect(mockEvent.preventDefault).toHaveBeenCalled()` 配对验证关闭流程副作用）
