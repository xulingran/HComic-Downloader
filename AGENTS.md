...
HComic Downloader 是一个基于 Electron + React + TypeScript 前端的漫画下载工具，Python 后端负责解析、下载、打包和维护。漫画以 CBZ 格式（含 ComicInfo.xml 元数据）保存，支持 hcomic、moeimg、jm、bika（哔咔）、copymanga、nh 共六个来源，并提供维护中心用于下载健康管理、孤儿临时目录清理和存储空间分析。除下载外，还内置本地漫画库（扫描、索引、阅读进度续读、元数据编辑）和在线阅读器（不下载即可浏览，含跳转预加载）。
...
│   ├── library_db.py — 漫画库索引 SQLite（可重建缓存；schema 不兼容/损坏时隔离旧库并重建，绝不修改漫画文件）
│   ├── library_indexer.py — 漫画库资产发现与索引（发现→解析→提交→对账四阶段，复用 maintenance/scanner 纯函数）
...
│   ├── ipc_server.py — IPCServer 类，Mixin 组合 python/ipc/ 下 12 个 Mixin
│   ├── ipc/ — 功能 Mixin（search/cover/preview/download/config/auth/migration/
│   │           history/favourite_tags/tag_list/maintenance/library）+ cover_cache/preview_cache/
│   │           library_cache/preview_executor/types/image_utils
...
│   ├── stores/ — 14 个 Zustand store（含 useLibraryStore / useLocalReaderStore）
│   ├── pages/ — Search/Download/History/Favourites/Maintenance/Settings/About/Toolbox
│   ├── components/ — common/favourites/library/maintenance/settings/tools
│   ├── hooks/ / utils/
...
- **sys.path 注入**：`python/ipc_server.py` 启动时把仓库根加入 `sys.path`，从而导入根目录的 Python 模块（`models`、`config`、`downloader`、`sources`、`library_db`、`library_indexer` 等）
...
- **本地漫画库**：`library_indexer.LibraryIndexer` 统一管理扫描生命周期（发现→解析→提交→对账），索引写入 `library_db.LibraryDB`（`~/.hcomic_downloader/library.db`，仅作可重建缓存，漫画文件始终是事实来源）。控制面 IPC 只返回内容哈希 / 媒体类型 / 资产版本，图片字节经 Electron `app-image://library/<sha256>` 协议按需流式交付，不进入 JSON-RPC 或渲染进程 JS 堆。预览图片请求由 `python/ipc/preview_executor.py` 的优先级队列执行器处理，跳页时目标页请求优先于历史队列。
...
- **本地漫画库**：单根目录模型，只索引 `downloadDir` 下内容。`library_db.LibraryDB` 与 `download_history` 相互独立，schema 不兼容或数据库损坏时隔离旧文件并完整重建，绝不修改漫画文件本身。资产 ID 为持久化随机 UUID，路径未变化时复用；页面 manifest 仅在首次阅读或资产版本变化时生成。真实路径与压缩包条目名只在后端使用，不通过列表 IPC 暴露。
