...
describe('MaintenancePage', () => {
  it('renders navigation tabs and default panel', () => {
    ...
    expect(screen.getByRole('button', { name: /临时目录清理/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /存储分析/i })).toBeInTheDocument()
    expect(screen.getByTestId('health-panel')).toBeInTheDocument()
  })

  it('switches to orphan cleanup panel', async () => {
    render(<MaintenancePage />)
    await userEvent.click(screen.getByRole('button', { name: /临时目录清理/i }))
    expect(screen.getByTestId('orphan-panel')).toBeInTheDocument()
  })

  it('switches to storage stats panel', async () => {
    render(<MaintenancePage />)
    await userEvent.click(screen.getByRole('button', { name: /存储分析/i }))
