...
describe('reader-jump-preload-priority: 跳转后目标页先于旧代请求完成', () => {
  ...
  it('target 跳转后新代邻居 urlHash 先于旧代残留写入共享缓存', async () => {
    ...
    await waitFor(() => expect(mockFetch.mock.calls.some(([url]) => url === 'u2')).toBe(true), { timeout: 2000 })
    const firstGeneration = mockFetch.mock.calls.find(([url]) => url === 'u2')?.[4] as number
    expect(firstGeneration).toBeGreaterThan(0)
    // 旧代请求悬挂，尚未写入缓存
    expect(result.current.imageCacheRef.current.get(1)).toBeUndefined() // u2 → idx 1

    // 2. 跳转到 target=10：推进到更高 generation
