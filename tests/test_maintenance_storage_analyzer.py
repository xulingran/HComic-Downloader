...
def test_analyze_orphan_files(tmp_path: Path):
    cbz = tmp_path / "orphan.cbz"
    _make_cbz(cbz)

    db = MagicMock()
    db.get_all_records.return_value = []  # no history

    result = analyze_storage(str(tmp_path), history_db=db)
    assert result["orphanFiles"]["count"] == 1
    assert result["orphanFiles"]["sizeBytes"] > 0
