...
def test_upsert_and_get_history(tmp_path):
    ...
    db.upsert(
        comic_id="1", title="Comic", cover_url="", source="hcomic",
        source_site="hcomic", media_id="", source_url="",
        last_page=3, total_pages=20,
    )
...
def test_history_stores_chapter_fields(tmp_path):
    db = _make_db(tmp_path)
    db.upsert(
        comic_id="999001", title="多章", cover_url="", source="JMCOMIC",
...
def test_chapter_fields_default_empty(tmp_path):
    db = _make_db(tmp_path)
    db.upsert(
        comic_id="2", title="No chapter", cover_url="", source="hcomic",
        source_site="hcomic", media_id="", source_url="",
        last_page=1, total_pages=10,
    )
