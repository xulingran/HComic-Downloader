...
def test_check_download_conflict_no_conflict(ipc_server, tmp_path):
    result = ipc_server.handle_check_download_conflict(
        comic_data={"id": "noconflict", "title": "No Conflict", "sourceSite": "hcomic"}
    )
...
def test_download_returns_conflict_without_overwrite(ipc_server, tmp_path):
    ...
    result = ipc_server.handle_download(
        comic_id="conflict2",
        comic_data={"title": "Conflict Comic 2", "sourceSite": "hcomic"},
    )
...
def test_download_proceeds_with_overwrite(ipc_server, tmp_path):
    result = ipc_server.handle_download(
        comic_id="overwrite1",
        comic_data={"title": "Overwrite Comic", "sourceSite": "hcomic"},
        overwrite=True,
    )
...
def test_download_no_conflict_proceeds_without_overwrite(ipc_server, tmp_path):
    result = ipc_server.handle_download(
        comic_id="nooverwrite",
        comic_data={"title": "No Overwrite Needed", "sourceSite": "hcomic"},
    )
    assert result["status"] in ("queued", "downloading")
    assert result["taskId"] is not None
