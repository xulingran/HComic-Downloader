...
def test_download_with_chapter_ids_creates_task_per_chapter(monkeypatch):
    ...
    result = server.handle_download(
        "999001", comic_data, chapter_ids=["999001", "999002"]
    )
...
def test_download_chapters_partial_failure_returns_created_and_failed(monkeypatch):
    ...
    server.parser.parsers = {
        "jmcomic": SimpleNamespace(get_chapter_images=flaky_get_chapter_images)
    }
    ...
    result = server.handle_download(
        "999001", comic_data, chapter_ids=["999001", "999002", "999003"]
    )

    # 第 1、3 章成功建任务，第 2 章失败被记录而非中断
    assert result["taskIds"] == ["999001", "999003"]
    assert [c.id for c in created] == ["999001", "999003"]
    assert result["failedChapters"] == [
        {"id": "999002", "name": "第 2 話", "error": "CDN 503"}
    ]
    assert result["status"] == "queued"
...
def test_download_chapters_all_failed_reports_error(monkeypatch):
    ...
    server._download_manager = SimpleNamespace(
        add_task=lambda comic, overwrite=False: comic.id, tasks={}
    )
    ...
    assert result["failedChapters"] == [
        {"id": "999001", "name": "第 1 話", "error": "boom"}
    ]
