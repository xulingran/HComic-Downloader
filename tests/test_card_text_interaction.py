...
from panels.comic_card import truncate_text_to_lines, get_card_key, is_title_expanded
...
class TestCardTextInteraction(unittest.TestCase):
    ...
    def test_truncate_title_to_three_lines(self):
        ...
        clipped, truncated = truncate_text_to_lines(long_text, font_obj, max_width=160, max_lines=3)
    ...
    def test_card_click_only_works_in_batch_mode(self):
        ...
        self.app.dl_ctrl.batch_select_mode_var.set(False)
        self.app.dl_ctrl.on_card_click(None, comic, frame)
        self.assertNotIn(comic, self.app.app_state.download.selected_comics)

        self.app.dl_ctrl.batch_select_mode_var.set(True)
        self.app.dl_ctrl.on_card_click(None, comic, frame)
        self.assertIn(comic, self.app.app_state.download.selected_comics)

    def test_title_expand_state_key(self):
        """展开状态按 comic_source:id 存储"""
        comic = ComicInfo(id="42", title="标题", comic_source="MMCG_SHORT")
        key = self.app.search_ctrl._get_card_key(comic)
        self.assertEqual(key, "MMCG_SHORT:42")
        self.assertFalse(self.app.search_ctrl._is_title_expanded(comic))
        self.app.app_state.search.card_title_expanded[key] = True
        self.assertTrue(self.app.search_ctrl._is_title_expanded(comic))
