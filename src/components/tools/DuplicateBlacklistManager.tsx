...
import type { DuplicateBlacklist, DuplicateBlacklistEntry } from '@shared/types'
import { useSettingsStore } from '@/stores/useSettingsStore'

interface DuplicateBlacklistManagerProps {
  /** 弹窗默认选中的来源（通常与检测来源一致） */
...
export function DuplicateBlacklistManager({
  defaultSource = 'hcomic',
  fingerprintToSize,
  onClose,
}: DuplicateBlacklistManagerProps) {
  ...
  const entries: DuplicateBlacklistEntry[] =
    (duplicateBlacklist as DuplicateBlacklist)[activeSource] ?? []
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-medium text-[var(--text-primary)]">已忽略的重复组</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {sources.map(s => {
            const count = (duplicateBlacklist as DuplicateBlacklist)[s.value]?.length ?? 0
            return (
              <button
                key={s.value}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {changed && (
                      <button
                        onClick={() => confirmMemberCount(activeSource, entry.fingerprint, currentSize!)}
                    ...
                    <button
                      onClick={() => removeDuplicateIgnore(activeSource, entry.fingerprint)}
