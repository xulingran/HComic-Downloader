...
`electron/login-window.ts` 当前对 hcomic / copymanga / jm 三个来源都实现了"自动检测登录 cookie → 自动关闭弹窗"的逻辑：各自通过导航监听或 cookie 轮询检测登录态，命中后调用 `completeLoginFlow` 提取 cookie，成功后延迟 5s 自动关窗。

该逻辑存在以下问题：

1. **用户预期不符**：登录完成后弹窗会"自己关掉"，用户无法确认登录状态、也无法主动控制何时提取凭证。
2. **jm 用户名提取脆弱**：`extractJmUsername` 依赖 DOM 访问，目前仅在 `completeLoginFlow`（关窗前）执行；现存的 `closed` 事件兜底路径在窗口销毁后无法访问 DOM，对 jm 是残缺的（用户名永远为空）。
...
| 提取事件 | `close`（非 `closed`） | `close` 触发时 DOM + session 均存活，可确定性提取 jm 用户名；`closed` 时窗口已销毁 |
...
- `bindJmLoginTracking`（jm cookie 轮询）
...
- `LoginWindowContext` 字段：`hasVisitedAuth0`、`jmUsername`、`clearSuccessTimeout`
...
用户点 ✕ → close 事件触发（DOM + session 存活）
  ...
  → 异步：jm 先 extractJmUsername（DOM），再 extractAndApplyCookies（session）
...
- 无 cookie / 缺关键登录态 cookie（jm `remember`、copymanga `token` 等）→ `notLoggedIn: true`
...
- **jm 用户名**：`close` 事件下 DOM 存活，需实测登录 jm → 关窗 → 收藏夹功能正常。
- **重入安全**：`settled` + `extractInProgress` 双标志防重入，需覆盖"提取中再次点 ✕"场景。
- **回归**：hcomic / copymanga / jm 三个来源各自走一遍"登录后关窗"与"未登录关窗"两条路径。
