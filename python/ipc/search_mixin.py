...
class SearchMixin:
    ...
    def handle_search(
        ...
        # language_filter 仅允许 NH / moeimg 使用 'chinese'；其他来源或非法值显式拒绝。
        effective_language_filter = ""
        if language_filter and effective_source in ("nh", "moeimg"):
            normalized = (language_filter or "").strip().lower()
            if normalized == "chinese":
                effective_language_filter = "chinese"
            else:
                raise ValueError(f"Unsupported language_filter for {effective_source}: {language_filter!r}")
        elif language_filter:
            raise ValueError("language_filter is only supported for sources nh and moeimg")
