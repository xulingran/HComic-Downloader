...
class CoverCacheDB:
    ...
    def __init__(
        ...
        # Add size column for existing databases lacking it
        ...
        # Pre-load recent entries into memory (respecting byte limit)
        ...
        logger.info(
            "Cover cache DB opened (%s), pre-loaded %d entries (%.1f MB / %d MB)",
            db_path,
            len(self._memory),
    ...
    def put(self, url: str, data_uri: str) -> None:
        ...
        with self._lock:
            ...
            while self._memory_bytes + sz > self._max_size_bytes and self._memory:
            ...
            if total_size > self._max_size_bytes:
                ...
                # Walk oldest entries, deleting until excess is freed
                rows = self._conn.execute(
